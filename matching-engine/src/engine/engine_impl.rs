...
impl MatchingEngine {
    ...
    fn find_order_place(order_book: &[Order], order: &Order) -> Option<usize> {
        ...
        for (index, order_from_book) in order_book.iter().enumerate() {
            ...
            } else if order.price == order_from_book.price
                && order.instant < order_from_book.instant
            {
                return Some(index);
            }
        }
    ...
    fn enqueue_order(order_book: &mut Vec<Order>, order: &Order, results: &mut Vec<OrderResult>) {
        ...
        if let Some(order_place) = order_place {
            order_book.insert(order_place, order.clone());
        }
    }

    fn match_order(booked_order: &Order, received_order: &Order) -> OrderMatchResult {
        #[allow(clippy::if_same_then_else)]
        let executed_price = if booked_order.order_type == super::OrderType::Buy
            && received_order.order_type == super::OrderType::Sell
            && booked_order.price >= received_order.price
        {
            booked_order.price
        } else if booked_order.order_type == super::OrderType::Sell
            && received_order.order_type == super::OrderType::Buy
            && booked_order.price <= received_order.price
        {
            booked_order.price
        } else {
        ...
        if remaining_qty > 0 {
            // partially executed
            remaining_order = Some(Order::new(
                received_order.order_type.clone(),
                remaining_qty,
                received_order.price,
                received_order.instant,
            ));
        }

        OrderMatchResult::new(
            Some(Order::new(
                received_order.order_type.clone(),
                executed_qty,
                executed_price,
    ...
    pub fn add_order(
        ...
        if order.order_type == super::OrderType::Buy {
            ...
            // order.order_type == super::OrderType::SELL
        ...
        while current_order.is_some() && match_order_book.is_empty() {
            let current_order_value: &Order = current_order.as_ref().unwrap();

            // match the order with the first order of the book
            let match_result: super::OrderMatchResult =
                MatchingEngine::match_order(&match_order_book[0], current_order_value);
            ...
            if match_result.executed_order.is_none() {
                ...
                if match_result.remaining_order.is_none()
                    && executed_order.quantity < original_order_from_book.quantity
                {
                    // the order dequeued from the book was partially executed
                    // we should put the remaining order back to its original position
                    match_order_book.insert(
                        0,
                        Order::new(
        ...
        if let Some(ref current_order) = current_order {
            MatchingEngine::enqueue_order(order_book, current_order, &mut results);
        }
